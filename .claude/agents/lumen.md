...
- Never run `npx impeccable craft` past `shape=pass` — stop at the confirmed design brief and route build to Atrium 🏛️ (Frontend Architect) and the implementing specialist
...
- Never run `npx impeccable live` without explicit per-invocation Curator 🗝️ (Project Lead) authorization
