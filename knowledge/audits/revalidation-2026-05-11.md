...
**Trigger:** Warden's revalidation of two standing findings from prior session baseline (2026-05-09):
...
- **Advisory #1115540:** Manual bump to `brace-expansion@>=1.1.13` — version bump proposal → Warden review → authorized `pnpm install`
- **Advisory #1115543:** Upgrade `eslint-config-next` and related TypeScript ESLint packages — version bump proposal → Warden review → authorized `pnpm install`
...
**Fix routing:** Atrium 🏛️ (Frontend Architect) — owns non-test `devDependencies` including build tooling (`vite`). Curator 🗝️ (Project Lead) to request version bump review upstream.
...
**[ADVISORY]** — Three moderate-severity transitive vulnerabilities remain unfixed:

1. **brace-expansion** (CVSS 6.5) — instances in both `eslint` and `eslint-config-next` chains
2. **yaml** (CVSS 4.3) — instance in `vite` chain

All three findings are eligible for upstream review and version bumps. Curator 🗝️ (Project Lead) should route to domain specialists for fix determination before Herald 📯 (Release Manager) stages any `pnpm-lock.yaml` changes that could be traceable to these paths.
...
| `yaml` CVE-2026-33532 | MODERATE | Atrium 🏛️ (Frontend Architect) | Review `vite` version bump upstream |
...
None — `.gitignore` already covers `.env*` patterns; the prior standing finding "bare `.env` not in `.gitignore`" was incorrect and is now closed.
...
2. Route yaml finding to Atrium 🏛️ (Frontend Architect) for upstream review of vite version options.
3. Document overrides (if Curator chooses to proceed despite advisory findings) in this report under each finding.
