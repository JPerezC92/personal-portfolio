...
**Pre-coding sync gate:** Before any implementing specialist writes or edits code, verify local branch is not behind `origin/main` — run `git fetch origin` then check `git status` for "behind" indicator. If behind, Herald 📯 (Release Manager) runs `git pull origin main` before any other git operation. Curator 🗝️ (Project Lead) owns this check; it runs once per session before delegating to any implementing specialist.
